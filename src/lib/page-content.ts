/* ==================================================================
   ...
   The words and pictures the storefront is built from, kept apart from
   the components that lay them out so a wording change is one edit in
   one file rather than a hunt through markup.

   These were editable from the admin panel for a while. The form was
   not worth the weight — a page of unlabelled boxes is a worse way to
   change a headline than editing this file. The `page_content` table
   is still in the schema, unused, so that decision can be revisited
   without a migration.
   ================================================================== */

/* ------------------------------------------------------------------
   The copy itself, one entry per page.
   ------------------------------------------------------------------ */

export const PAGE_CONTENT: Record<string, Record<string, unknown>> = {
  home: {
    hero: {
